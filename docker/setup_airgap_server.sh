...
# Air Gapped Server Setup Script for Docker
# Version: 2.4
# Description: This script automates the setup of an air-gapped server for DroneEngage inside a Docker container.
# Author: Your Name
...
#IP='192.168.1.161'
IP=$(hostname -I | awk '{print $1}') # or IP=$(ip route get 1.2.3.4 | awk '{print $7}')
...
echo -e $YELLOW "For more details: https://youtu.be/R1BedRTxuuY" $NC
...
echo -e $YELLOW "SSL name should be localssl.crt and localssl.key at ${PWD}" $NC
...
echo "export NODE_EXTRA_CA_CERTS=/root/ssl/root.crt" | sudo tee -a /etc/profile
...
sudo tee /etc/turnserver.conf <<EOL
listening-port=3478
...
echo -e $YELLOW "For more info, check this video: https://youtu.be/ppwuUqomxXY" $NC
popd
...
echo -e $YELLOW "You can access the web client at https://${DOMAIN_NAME}:8001" $NC
